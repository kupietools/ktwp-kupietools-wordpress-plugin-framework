...
Add namespaced class instead of having to do a text replacement across files for PLUGIN_PREFIX - once it tests fully working.

Move all user-customizable sections to an array of configuration variables at top

Move top tab position to Kupietools settings panel
...
// Add THIS plugin's section
    add_action('kupietools_sections', function() {
        ?>
        <details class="card ktwp-settings-card PLUGIN_PREFIX-settings-card" style="max-width: 800px; padding: 20px; margin-top: 20px;" open="true">
...
/* * * * * * SECTION 2: Create UI Tab if desired. Otherwise remove this section * * * * */

// Add the control panel HTML and required scripts/styles

function PLUGIN_PREFIX_add_tab() {
 ?>
    <div id="PLUGIN_PREFIX-control" class="PLUGIN_PREFIX-control"> /* setup script below will add generic KTWP tab class to this as it makes it visible*/
    ...
    <script id="PLUGIN_PREFIX_inline_tab_script">
...
PLUGIN_PREFIX_thisTab.style.display = "block"; /* don't display until positioned, because CloudFlare rocket loader runs this script noticeably long after it's originally drawn in its original position, so the tab moves. */
...
add_action('wp_footer', 'PLUGIN_PREFIX_add_tab');

/* * * * * * END SECTION 2: Create UI Tab if desired. Otherwise remove this section * * * * */

/* * * * * * SECTION 3: if you want to display the panel in a wordpress page with a shortcode "PLUGIN_PREFIX_displayPanel", this will let you. Remove this section if not needed. * * * * */
add_shortcode("PLUGIN_PREFIX_displayPanel","PLUGIN_PREFIX_panel_content");
/* * * * * * END SECTION 3: shortcode * * * * */



// Add the popup control panel HTML to the footer
//function to generate the plugin panel content

function PLUGIN_PREFIX_panel_content($args = [], $content = null /* you can remove these parameters if you're not using a shortcode */) {
/* SUB-SECTION 3A: you can remove this shortcode parameter block if you're not using a shortcode */
$args = shortcode_atts(
        [
        ...
        $args,
        "PLUGIN_PREFIX_panel_content"
    );
 /* end of SUB-SECTION 3A: shortcode parameter block */   
?>
...
<?php PLUGIN_PREFIX_panel_content(); ?>
       </div>
<script id="PLUGIN_PREFIX_inline_panel_script">
/* stop event listeners from bubbling */
const PLUGIN_PREFIX_thisPanel=document.getElementById("PLUGIN_PREFIX-panel");
if (PLUGIN_PREFIX_thisPanel) 
{
['mousedown','mouseover','mousemove','dragstart','touchstart','touchmove','click'].forEach(
thisAction => {
PLUGIN_PREFIX_thisPanel.addEventListener(thisAction, function(event) 
{ event.stopPropagation();}
);
});
}

/* move the new tab to stack under tabs from other KupieTools plugins, if desired (which it probably is.) Remove this section if you don't want a tab. */ 

const PLUGIN_PREFIX_thisPanel=document.getElementById("PLUGIN_PREFIX-panel");
if(PLUGIN_PREFIX_thisPanel) {
PLUGIN_PREFIX_thisPanel.style.top = (140 + if(typeof PLUGIN_PREFIX_numOfKupieTabs==='undefined') {0} else {PLUGIN_PREFIX_numOfKupieTabs}  * 38) + "px";
...
function PLUGIN_PREFIX_enqueue_css() {
    wp_enqueue_style('PLUGIN_PREFIX-styles', plugins_url('css/style.css', __FILE__));
}
add_action('wp_enqueue_scripts', 'PLUGIN_PREFIX_enqueue_css');
